...
*   **Vendor Dashboard Page (`/dashboard`)**:
    *   **Content:** A tabbed interface for "Overview" and "Bookings". The overview includes stats on revenue, bookings, and ratings, plus a table of all the vendor's listings. The bookings tab shows incoming requests that can be approved or declined.
    *   **Function:** The vendor's central hub for managing their business on the platform.

*   **Profile Page (`/profile`)**:
    *   **Content:** Displays user information (name, email, member since). Below are sections with links to account management, settings, and business tools (if the user is a vendor). Includes a prominent "Log Out" button and a flow for users to become vendors.
